...
/-! # Equivalence of Context-Sensitive and Non-Contracting Grammars

This file establishes the equivalence between context-sensitive grammars
(rules of the form αAβ → αγβ with γ non-empty) and non-contracting grammars
(unrestricted grammars where |rhs| ≥ |lhs| for every rule).

## Overview

A **context-sensitive grammar** has rules `αAβ → αγβ` where `A` is a nonterminal,
`α`, `β` are strings of symbols (the context), and `γ` is a non-empty string.
...
grammar can be converted to an equivalent context-sensitive grammar (hard direction).
...
- `is_CS_iff_is_noncontracting` — the two language classes coincide
...
/-! ## CS grammars are non-contracting -/
...
/-- Every context-sensitive language is a non-contracting language. -/
  ...
  · -- Apply the `nc_unlift_one` lemma to transform `[nt (inr (inl t))]` to `[terminal t]`.
    have h_transform : CS_transforms (csg_of_noncontracting g hg)
      ([.nonterminal (.inr (.inl t))] ++ (w.map (fun t => .nonterminal (.inr (.inl t))))) ([.terminal t] ++ (w.map (fun t => .nonterminal (.inr (.inl t))))) := by
        convert nc_unlift_one g hg t ( hw t ( by simp +decide ) ) [ ] ( map ( fun t => symbol.nonterminal ( Sum.inr ( Sum.inl t ) ) ) w ) using 1;
    ...
    have h_ind : CS_derives (csg_of_noncontracting g hg)
      ([symbol.terminal t] ++ (w.map (fun t => .nonterminal (.inr (.inl t))))) ([symbol.terminal t] ++ (w.map symbol.terminal)) := by
        convert CS_deri_with_context [ symbol.terminal t ] [] ( ih fun t ht => hw t ( List.mem_cons_of_mem _ ht ) ) using 1;
        · simp +zetaDelta at *;
        · simp +zetaDelta at *;
    ...
    have h_phase1 : CS_derives (csg_of_noncontracting g hg) (pattern.map nc_liftSym) ((List.range (n - 1)).map (nc_aux ri) ++ [nc_liftSym (pattern.getD (n - 1) (symbol.nonterminal r.input_N))]) := by
      ...
      have h_phase1_seq : ∀ (k : ℕ), k ≤ n - 1 → CS_derives (csg_of_noncontracting g hg) (pattern.map nc_liftSym) ((List.range k).map (nc_aux ri) ++ [nc_liftSym (pattern.getD k (symbol.nonterminal r.input_N))] ++ (pattern.drop (k + 1)).map nc_liftSym) := by
        ...
        · convert CS_deri_of_deri_deri ( ih ( Nat.le_of_succ_le hk ) ) ( CS_deri_of_tran ( h_phase1 k ( Nat.lt_of_succ_le hk ) ) ) using 1;
          rw [ show drop ( k + 1 ) pattern = [ pattern.getD ( k + 1 ) ( symbol.nonterminal r.input_N ) ] ++ drop ( k + 1 + 1 ) pattern from ?_ ] ; simp +decide [ List.map_append ];
